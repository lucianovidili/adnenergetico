...
const requestConstURL = 'http://localhost:8080/adnenergetico/cartabazi/';
...
function aAnimalsExplanationHandler(e) {
    ...
    let para = document.getElementById(pilarId).textContent;    //Rata o Perro o Gallo, etc.
...
function imgAnimalClickHandler(e) {
    ...
    let para = document.getElementById(pId).textContent;
    sessionStorage.setItem('pressedAnimal', para);
...
function populateAll(animales, alsoFullDate) {
    ...
    function populateParas(animales) {
        ...
        let pDefensa = document.getElementById('pDefensa');
        let pPoder = document.getElementById('pPoder');
        pEnvoltura.textContent = animales["envoltura"];
        pTronco.textContent = animales["tronco"];
        pDefensa.textContent = animales["defensa"];
        pPoder.textContent = animales["poder"];
    }
}
...
function saveState() {
    ...
    let pDefensa = document.getElementById('pDefensa');
    let pPoder = document.getElementById('pPoder');
    sessionStorage.setItem('envoltura', pEnvoltura.textContent);
    sessionStorage.setItem('tronco', pTronco.textContent);
    sessionStorage.setItem('defensa', pDefensa.textContent);
    sessionStorage.setItem('poder', pPoder.textContent);
}
...
function btnRHandler() {
    //Handler cuando se apreta el boton R para "resetear/reiniciar" todo (Es como si hubieran ingresado a la pagina por primera vez).
